...
Dirty.prototype.set = function(key, val, cb) {
  if (val === undefined) {
    ...
    //if (this._docs[key] === undefined) { // TODO check that correct
      this._keys.push(key);
    }
    this._docs[key] = val;
  }
...
Dirty.prototype.compact = function() {
  if (!this.path) {
    return;
  }
  ...
  if (this.loaded) {
    ...
    this.once('load', this._maybeCompact);
  }
};

Dirty.prototype._maybeCompact = function() {
  this._maybeFlush();

  if (this.flushing) {
    this.once('drain', this._doCompact);
  } else {
    this._doCompact();
  }
};

Dirty.prototype._doCompact = function() {
  // var origSet = Dirty.prototype.set;
  // Dirty.prototype.set = function() {
  //   // buffer writes
  // };
  // Dirty.prototype.set = origSet;
  var sourcePath = this.path;
  console.log('Would compact %s', sourcePath);
  var counter = 0;
  var targetPath;
  function getTargetPath() {
    targetPath = sourcePath + '.' + ++counter;
  ...
  // for (var i = 0; i < this._keys.length; i++) {
  //   var key = this._keys[i];
  //   out += JSON.stringify({key: key, val: this._docs[key]})+'\n';
  // }
  ...
  console.log('Compacting ready, should restart the process to open the new db.');
};
