...
// TODO mix some of these in from core

function initShow(block) {

  // Show button functionality
  if (__CONTROL__) {
    //blockbase.setShowButtons.call(block, block.$toolbar);
  ...
  block.on('change:visible', function(shown, firstLoad) {
    ...
    if (__CONTROL__ && shown) {
      if (firstLoad) {
        ...
        this.$el.finish().animate({opacity: 1}, 200, function() {
          $(this).css('opacity', '');
        });
      }
    }
    if (__CONTROL__ && !shown) {
      if (firstLoad) {
        ...
        this.$el.finish().animate({opacity: 0.5}, 200);
      }
    }
    ...
    if ((__WEB__ || __STAGE__) && shown)  {
      if (firstLoad) {
        ...
        //this.$el.stop(true).animate({height: 'show'}, 400).animate({opacity: 1}, 400, function() {
        this.$el.finish().animate({height: 'show'}, 200).animate({opacity: 1}, 200, function() {
          $(this).css('opacity', '');
        });
...
//        this.$el.stop(true).animate({opacity: 0}, 400, function() {
//          self.$el.stop(true).animate({height: 'hide'}, 400);
          // TODO just copied from mixer-pre
...
function initSelect(block) {
  ...
  if (__CONTROL__) {
    //blockbase.setSelectButtons.call(block, block.$toolbar);
...
function initDelete(block) {
  if (__CONTROL__) {
    //blockbase.setDeleteButton.call(this, this.$toolbar);
...
function initLift(block) {
  if (__CONTROL__) {
    //blockbase.setLiftButton.call(this, this.$toolbar);
