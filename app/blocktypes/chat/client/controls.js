...
function initShow(block) {
  ...
  block.on('change:visible', function(shown, firstLoad) {
    if (block.config.blockGroup) {
      // TODO perhaps parse hash into a structured object
      var currentBlockGroup = hash.getHash();
      if (currentBlockGroup && currentBlockGroup !== block.config.blockGroup) {
        console.log('hiding!')
        shown = false;
      }
    }
    ...
    if (__CONTROL__ && !shown) {
      block.$el.removeClass('core-onweb');
      if (firstLoad) {
  ...
  block.on('change:blockGroup', function(blockGroup) {
    // Wait until all config keys are updated
    block.once('configUpdated', function() {
      // Possible problem: double triggering if changed when reconnecting, may flash
      block.emit('change:visible', block.config.visible);
    });
  });
...
function initSelect(block) {
  ...
  block.on('change:selected', function(selected) {
    ...
    if ((__CONTROL__ || __WEB__ || __STAGE__) && selected)  {
      //block.$el.css('background-color', '#FFE');
    }
    if ((__CONTROL__ || __WEB__ || __STAGE__) && !selected)  {
      //block.$el.css('background-color', '');
    }
  });
  block.emit('change:selected', block.config.selected);
};

function initExport(block) {
  if (__CONTROL__) {
    var $exportButton = blockbase.newExportButton(block);
    block.$toolbar.find('#' + block.id + '-export').replaceWith($exportButton);
  }
};
...
function initDelete(block) {
  ...
  // TODO later: Clear timers and intervals, etc.
};

function initBlockGroup(block) {
  if (__CONTROL__) {
    var $blockGroupButton = blockbase.newBlockGroupButton(block);
    block.$toolbar.find('#' + block.id + '-blockGroup').replaceWith($blockGroupButton);
  }
};
...
if (__CONTROL__ || __STAGE__) {
  exports.participantCount = function(block) {
    ...
    // TODO better
    //if (__CONTROL__ || __STAGE__) {
      // Initiate, later get with rest of data
    //  this.rpc('$getPC', function(err, participantCount) {
    //    block.$PCin(participantCount);
    //  });
    //}
  };
}
