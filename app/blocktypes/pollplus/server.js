...
Block.prototype.__getBlockFrontendConfigForChannelUser = function(channel, user) {
  var staticConfig = {
    ...
    options: this.options,
    highlightedOption: this.frontends.highlightedOption,
    allowMultiple: this.frontends.allowMultiple
  };

  // Send user's own votes
  if (channel.type === 'web' || channel.type === 'control' || channel.type === 'stage') {
    var voterId = channel.id + ':' + (user && user.id);
    if (this.voters[voterId]) {
      staticConfig.ownVotes = this.voters[voterId];
    }
  }
  ...
  if (channel.type === 'control' || channel.type === 'screen') {
...
Block.prototype.$clear = function(req) {
  ...
  this.rpc('screen:$setConfig', {participantCount: this.participantCount});

  for (var channelId in this.channels) {
    if (this.channels[channelId].type !== 'web') {
      this.rpc(channelId + ':$setConfig', {resultMeta: this.resultMeta, results: this.results});
    } else {
      this.rpc(channelId + ':$setConfig', {ownVotes: {}});
    }
  }
...
Block.prototype.$allowMultiple = function(req, allowMultiple) {
  if (req.channel.type !== 'control') return;
  allowMultiple = !!allowMultiple;
  if (this.frontends.allowMultiple !== allowMultiple) {
    this.frontends.allowMultiple = allowMultiple;
    //this.saveFrontends();
    this.save();
    this.rpc('$setConfig', {allowMultiple: this.frontends.allowMultiple});
    console.info({
      userId: req.user.id,
      channelId: req.channel.id,
      blockId: this.id,
...
Block.prototype.$highlightOption = function(req, optionId) {
  ...
  if (this.frontends.highlightedOption === optionId) {
    this.frontends.highlightedOption = null;
  } else {
    this.frontends.highlightedOption = optionId;
  }

  for (var channelId in this.channels) {
    if (this.channels[channelId].type !== 'web') {
      this.rpc(channelId + ':$setConfig', {highlightedOption: this.frontends.highlightedOption});
    }
  }
...
Block.prototype.$vote = function(req, optionId) {
  ...
  if (!this.voters[userId]) {
    ...
    this.voters[userId] = { vote: null, votes: [] };
  ...
  if (this.frontends.allowMultiple) {

    if (!voter.votes) {
      voter.votes = [];
    }
    var alreadyExistingIndex = voter.votes.indexOf(optionId);
    if (alreadyExistingIndex !== -1) {
      voter.votes.splice(alreadyExistingIndex, 1);
    } else {
      voter.votes.push(optionId);
    }

  } else {

    if (voter.vote === optionId) {
      // Allow detoggling
      voter.vote = null;
    } else {
      // Change vote
...
Block.prototype.calcRanking = function() {
  ...
  for (var voterKey in this.voters) {
    ...
    if (this.frontends.allowMultiple) {
      var votesArray = this.voters[voterKey].votes;
      if (votesArray != null && votesArray.length) {
        voterCount++;
        for (var j = 0; j < votesArray.length; j++) {
          var vote = votesArray[j];
          results[vote].points++;
        }
      }
    } else {
      if (this.voters[voterKey].vote != null) {
        voterCount++;
...
//Block.prototype.$getPC = function(req) {
//  req.reply(null, this.participantCount);
//};
...
Block.prototype.$getData = function(req) {
  if (req.channel.type == 'control' || req.channel.type == 'stage' || req.channel.type == 'screen') {
    ...
    var ownVotes = [];
    req.reply({options: this.options, ownVotes: ownVotes});
  }
};
